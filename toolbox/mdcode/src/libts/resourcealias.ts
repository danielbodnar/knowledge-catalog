...
export class ResourceAlias {
  ...
  add(alias: string, resourceType: string, resource: string) {
    if (this._defaultAlias.has(alias)) {
      throw new Error(`Cannot define predefined alias ${alias}, which is predefined for ${this._defaultAlias.get(alias)!}`);
    }
  
    if (this.customAlias.has(alias)) {
      throw new Error(`Duplicate alias defined: ${alias}, which has a definition for ${this.customAlias.get(alias)!}`);
    }

    if (!(Object.values(ResourceType).includes(resourceType as ResourceType))) {
      throw new Error(`Unrecognized resource type ${resourceType}, supported types are [ ${Object.values(ResourceType).join(", ")} ]`);
    }

    const resourceTypeName = `${resourceType}: ${resource}`;

    if (this._defaultResource.has(resourceTypeName) || this.customResource.has(resourceTypeName)) {
      const duplicate = this._defaultResource.has(resourceTypeName) ? this._defaultResource.get(resourceTypeName)! : this.customResource.get(resourceTypeName)!;
      throw new Error(`Cannot re-define resource ${resource}, which has an alias as ${duplicate}`);
    }
  ...
  lookupAlias(alias: string, resourceType: ResourceType): string {
    if (this._defaultAlias.has(alias) && this._defaultAlias.get(alias)!.startsWith(resourceType)) {
